...
public class SolutionDGMLFileBuilder extends ADGMLFileBuilder<ISolution, Configuration> {
  ...
  protected void buildDirectedGraph(
    ...
    for (Group aGroup : allGroups) {
      createNode(
          getId(aGroup, configuration), aGroup.getName(), true, true, directedGraph, nodeByIds);
      for (ISolutionItem aSolutionItem : aGroup.getProjects()) {
        createNode(
            getId(aSolutionItem, configuration),
            aSolutionItem.getName(),
            false,
            false,
            directedGraph,
            nodeByIds);
    ...
    for (Group aGroup : allGroups) {
      for (Group aSubGroup : aGroup.getSubGroups()) {
        createLink(
            getId(aGroup, configuration),
            getId(aSubGroup, configuration),
            directedGraph,
            containsCategory,
            nodeByIds);
      }
      for (ISolutionItem aSolutionItem : aGroup.getProjects()) {
        ...
        for (ISolutionItem aProjectDependency : aSolutionItem.getProjectDependencies()) {
          createLink(
              getId(aSolutionItem, configuration),
              getId(aProjectDependency, configuration),
              directedGraph,
              null,
              nodeByIds);
        }
