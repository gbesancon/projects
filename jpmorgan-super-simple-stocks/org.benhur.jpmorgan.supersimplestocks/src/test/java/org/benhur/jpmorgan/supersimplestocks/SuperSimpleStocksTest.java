...
public class SuperSimpleStocksTest {
  ...
  public void testRequirements() {
    ...
    for (Stock stock : gbceTradingPlace.stocks) {
      assertEquals(
          dividendYield[iStock],
          StockFormulas.ComputeDividendYield(stock, marketPrice[iStock]),
          DELTA);
      assertEquals(
          peRatio[iStock], StockFormulas.ComputePERatio(stock, marketPrice[iStock]), DELTA);
      assertEquals(
          vwsp1[iStock],
          StockFormulas.ComputeVolumeWeightedStockPrice(
              TradingService.filterTradesByTimestamp(
                  TradingService.filterTradesByStock(gbceTradingPlace.trades, stock),
                  15 * 60 * 1000,
                  15 * 60 * 1000)),
          DELTA);
      assertEquals(
