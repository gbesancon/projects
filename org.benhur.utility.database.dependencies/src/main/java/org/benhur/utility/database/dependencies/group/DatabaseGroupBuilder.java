...
public class DatabaseGroupBuilder {
	...
	public Group buildGroup(IDatabase database, List<ITable> tables, Configuration configuration) {
		Group group = new Group(database.getId(), database.getName(),
	...
	protected Group createGroupFromConfiguration(int[] ids, Configuration configuration) {
		...
		try {
			...
			if (!groupName.equalsIgnoreCase("")) {
				...
				group = new Group(groupName, groupName, table -> RegExUtility.checkValue(table.getName(),
						groupProjectNameIncludePatternString, groupProjectNameExcludePatternString));
			}
	...
	protected List<Group> createGroupsFromDatabase(IDatabase database) {
		...
		for (ICatalog catalog : database.getCatalogs()) {
			Group catalogGroup = new Group(database.getId() + "_" + catalog.getId(), catalog.getName(),
					table -> table.getSchema().getCatalog() == catalog);
			for (ISchema schema : catalog.getSchemas()) {
				Group schemaGroup = new Group(schema.getId(), schema.getName(), table -> table.getSchema() == schema);
				...
				for (String tableType : tableTypes) {
					Group tableTypeGroup = new Group(schema.getId() + "_" + tableType, tableType,
