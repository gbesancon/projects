...
class Player {
	...
	class Position {
		...
		public int computeDistance(Position position) {
			return (int) Math.sqrt((position.x - x) * (position.x - x) + (position.y - y) * (position.y - y));
		}
	...
	class Goal {
		...
		public Position getCenter() {
			return new Position((pole0.position.x + pole1.position.x) / 2, (pole0.position.y + pole1.position.y) / 2);
		}
	}
	...
	class Map {
		...
		public final Goal goal0 = new Goal(new Pole(new Position(0, 1750), 150), new Pole(new Position(0, 5750), 150));
		public final Goal goal1 = new Goal(new Pole(new Position(16000, 1750), 150),
				new Pole(new Position(16000, 5750), 150));
	...
	public Map readInput(Scanner in) {
		...
		for (int i = 0; i < entities; i++) {
			...
			case "WIZARD":
				map.wizards
						.add(new Wizard(entityId, new Player.Position(x, y), new Player.Velocity(vx, vy), state == 1));
				break;
			case "OPPONENT_WIZARD":
				...
				break;
			case "SNAFFLE":
				map.snaffles.add(new Snaffle(entityId, new Player.Position(x, y), new Player.Velocity(vx, vy)));
				break;
			case "BLUDGER":
				map.bludgers.add(new Bludger(entityId, new Player.Position(x, y), new Player.Velocity(vx, vy)));
				break;
	...
	class Strategy {
		...
		public Snaffle computeClosestSnaffleForWizard(Wizard wizard, Map map, List<Snaffle> excludedSnaffles) {
